...
export function validerFAIAInput(input) {
  ...
  if (!Array.isArray(accounts)) erreurs.push('accounts doit être un tableau');
  else if (accounts.length === 0) avertissements.push('accounts vide : MasterFiles/GeneralLedgerAccounts sera vide');

  const accountIds = new Set(accounts.map((a) => String(a.id)));
  for (const a of accounts) {
    if (!a.id) erreurs.push('un compte sans id détecté');
    if (!a.label) avertissements.push(`compte ${a.id} sans label`);
  }

  // --- Entries (équilibre D=C) ---
  const entries = input.entries || [];
  if (!Array.isArray(entries)) erreurs.push('entries doit être un tableau');
  let i = 0;
  ...
  // --- SalesInvoices : si présentes, contrôles légers ---
  const sales = input.salesInvoices || [];
  for (const s of sales) {
    if (!s.invoiceNo) erreurs.push(`salesInvoice sans invoiceNo`);
    if (!validerDateISO(s.date)) erreurs.push(`salesInvoice ${s.invoiceNo || ''} date invalide`);
  }
  const purchases = input.purchaseInvoices || [];
  for (const ph of purchases) {
    if (!ph.invoiceNo) erreurs.push(`purchaseInvoice sans invoiceNo`);
    if (!validerDateISO(ph.date)) erreurs.push(`purchaseInvoice ${ph.invoiceNo || ''} date invalide`);
  }
